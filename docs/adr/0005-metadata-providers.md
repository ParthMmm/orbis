...
- **Enrich on every save, including a save with a typed title.** Better metadata on the desktop path, and rejected because it puts a network call inside a save that has always been local. A provider stall would become a stalled save. `docs/plans/native-clients-program.md` lane one of unit U3 originally expected this behaviour and was reworded to match.
...
- **Keep the YouTube key in the deployment checkout or the repository.** Rejected because the deployment checkout is a copy of the repository at a branch, so a secret there is one `git status` from being committed. It reaches the service through a systemd `EnvironmentFile` drop-in pointing at the working checkout, where `.env` files are already ignored.

...
- [x] Lane 1. Regression lane against trunk. Reworded during implementation. The original wording expected a save that supplies a title to also come back enriched, which contradicts the decision that a typed title skips the provider call so the desktop client keeps a save path with no network in it. See `docs/adr/0005-metadata-providers.md`. Run two saves at head, one with a title and one with a bare Source Link. Pass when the titled save stores exactly the title it was given and reports pending metadata, and the bare save stores the provider's title plus creator, artwork, and duration. Verified against the deployed service rather than a screenshot, which is why this box carries no artifact name.
